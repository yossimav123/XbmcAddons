...
def AddContextItem(liz, url, name):
    commands = []
    runner = "XBMC.RunPlugin(%s?mode=10&url=%s)"%(sys.argv[0], url)
    # print runner
    commands.append(( "Download", runner, ))
    # "XBMC.RunPlugin(%s?mode=1&fid=%s)"%(sys.argv[0], n)
    liz.addContextMenuItems( commands )
    
def addVideoLink(name, url, vparams, mode, iconimage='DefaultFolder.png', summary = ''):
        u = sys.argv[0] + "?url=" + urllib.quote_plus(url) + "&mode=" + str(mode) + "&name=" + name
        liz = xbmcgui.ListItem(name, iconImage=iconimage, thumbnailImage=iconimage)
        liz.setInfo(type="Video", infoLabels={ "Title": urllib.unquote(name), "Plot": urllib.unquote(summary)})    
        liz.setProperty('IsPlayable', 'true')
        AddContextItem(liz, urllib.quote_plus(url) +vparams, name)
